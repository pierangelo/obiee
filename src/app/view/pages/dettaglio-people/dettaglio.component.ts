...
export class DettaglioComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.optionInstoCategoria.series[0].tooltip = {
      formatter: function(d) {
        return (
          d.name +
          '<br/><span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color: #5fd3e6"></span>' +
  ...
  private loadData() {
    ...
    this.dettaglioService.getDipendentiPerCategoria(
      this.chartIstoCategoria,
      this.optionInstoCategoria
    );

    this.dettaglioService.getDipendentiPerQualifica(
      this.chartIstoQualifica,
      this.optionInstoPerQualifica
    );

    this.dettaglioService.getDipendentiPerTipoImpiego(
      this.chartIstoDip,
